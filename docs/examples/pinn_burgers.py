...
# condition estimate to one of a model trained without a physics-based loss.

# We will assume knowledge of:
# - The boundary conditions, \\\(u(1, t) == u(-1, t) == 0.0\\\) (and will build this directly into our model and prior)
# - The form of burgers equation \\\(\frac{\partial u}{\partial t} + u\frac{\partial u}{\partial t} = \nu \frac{\partial^{2} u}{\partial x^{2}}\\\)
...
# We will spatial discretise \\\(x\in[-1, 1]\\\) to reduce the Burger PDE into a system of ODEs.
...
sol = diffrax.diffeqsolve(
    ...
    dt0=δt,
    t1=t1,
    y0=-jnp.sin(x_grid[1:-1] * jnp.pi),  # initial conditions
    args=(1e-2 / jnp.pi, x_grid[1] - x_grid[0]),
...
# plt.scatter(*x[:, ::-1].T, c=y, cmap="inferno", s=4, edgecolors="k")
plt.xlabel("t")
...
y = us.T[x_slice, t_slice].reshape(-1, 1)

eps = 1e-2 * jr.normal(key=jr.PRNGKey(seed=20260527), shape=y.shape)
...
class BurgerPrior(RBFPrior):
    ...
    def __init__(self):
        super().__init__(lengthscales=(0.2,), variance=1.0)

    def wx(self, x: jnp.ndarray) -> jnp.ndarray:
        return 1 - x**4  # enforces boundary conditions
    ...
    def __call__(
        ...
        *,
        ...
        def _burgers_physics_loss(xt: jnp.ndarray, key: jnp.ndarray) -> jnp.ndarray:
            ...
            return ut_fn(x, t, key) + u(x, t, key) * ux_fn(x, t, key) - nu * uxx_fn(x, t, key)
        ...
        physics_loss = jnp.mean(physics_loss**2)
        # jax.debug.print("fsvi_loss: {}, phys_loss: {}", tractable_fsvi_loss, physics_loss)
...
# Now everything's ready to train some models! First, we'll train a model without the physics-informed term at inspect
# its predictions for the initial conditions.

# %%

logdir = "/tmp/pinn_burgers/" + datetime.now().strftime("%Y%m%d-%H%M%S")
trainer = Trainer(
...
def _plot_model_ic(m: TrainingModel, n_samples: int = 256) -> None:
    x_plot = jnp.linspace(-1, 1, 101)
    t0 = jnp.zeros_like(x_plot)
    fig, ax = plt.subplots(1, 1, figsize=(4, 4))
    xx = jnp.stack([x_plot, t0], axis=-1)
    pred_f_samples = m.net.predict_f_samples(xx, n_samples, key=key)  # [S, N, O]
    ...
    ax.plot(x_plot, 1.95 * (1 - x_plot**2), ls=":", c="k")
    ax.plot(x_plot, -1.95 * (1 - x_plot**2), ls=":", c="k")
    ax.plot(x_plot, pred_f_mean, c="k", lw=0.8)
    ax.fill_between(
        x_plot,
    ...
    ax.plot(x_plot, ymean, c="#59A14F", lw=0.8, ls="--")
    ax.plot(
        x_plot, ymean + 1.95 * ystd, c="#59A14F", lw=0.66, ls="--"
    )  # epistemic + aleatoric uncertainty
    ax.plot(x_plot, ymean - 1.95 * ystd, c="#59A14F", lw=0.66, ls="--")
    for ii in range(4):
        ax.plot(x_plot, pred_f_samples[ii, :, 0], c="#4E79A7", lw=0.66, alpha=0.66)
    ax.set_ylim(-2.8, 2.8)


_plot_model_ic(m_no_physics, n_samples=256)
...
# %%
_plot_model_ic(m_physics, n_samples=256)
