...
def _main(cfg: DictConfig) -> None:
    ...
    ds_x_transform = instantiate_assert_type(cfg.ds_x_transform, DataTransformFn)
    x_transform = ds_x_transform.fit(ds.x)
    ds = TransformedTensorDataset(ds, xbijector=x_transform)

    val_ds = TransformedTensorDataset(
        ds=instantiate_assert_type(cfg.val_ds, TensorDataset), xbijector=x_transform
    )
    context_ds = TransformedTensorDataset(
        instantiate_assert_type(cfg.context_ds, TensorDataset),
        xbijector=x_transform,
    )
    ood_ds = TransformedTensorDataset(
        instantiate_assert_type(cfg.ood_ds, TensorDataset),
        xbijector=x_transform,
    )

    detect_ood_ds = TransformedTensorDataset(
        ds=TensorDataset(
    ...
    sampler = instantiate_assert_type(cfg.sampler, Sampler, data=context_ds)
